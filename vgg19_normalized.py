...
class Vgg19Normalized:
    def __init__(self, vgg19_npy_path=None):
        if vgg19_npy_path is None:
            path = inspect.getfile(Vgg19Normalized)
            path = os.path.abspath(os.path.join(path, os.pardir))
            path = os.path.join(path, "vgg19_normalized_V1proto.npy")
            vgg19_npy_path = path
    ...
    def get_conv_filter(self, name):
        print('conv:', self.data_dict[name]["weights"].shape)
        return tf.constant(self.data_dict[name]["weights"], name="filter")

    def get_bias(self, name):
        print("bias:", self.data_dict[name]["biases"].shape)
